...
for query_vector in user_viewed_vectors:
    ...
    for i in range(len(indices[0])):
        # all_similar_products.append((indices[0][i], distances[0][i]))
...
# Получаем входные данные от Node.js
user_viewed_vectors = numpy.array(json.loads(sys.argv[1]), dtype='float32')  # Эмбеддинги просмотренных товаров
all_product_vectors = numpy.array(json.loads(sys.argv[2]), dtype='float32')  # Все эмбеддинги товаров

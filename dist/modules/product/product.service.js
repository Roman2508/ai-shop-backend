...
let ProductService = class ProductService {
    ...
    async paginateAndFilter(query) {
        const filtredEntries = Object.entries(query).filter(([_, value]) => value !== null && value !== undefined && value !== '');
        const filtredQuery = Object.fromEntries(filtredEntries);
        const { sortBy, limit, skip, priceFrom, priceTo, ...filterParams } = filtredQuery;
        const filter = [];
        const order = {};
        if (sortBy) {
            if (sortBy === 'price:asc') {
        ...
        if (Object.keys(filterParams).length) {
            ...
            for (const key in filterParams) {
                if (keys.includes(key)) {
                    ...
                    const queryFilter = query.map((q) => ({ [key]: { contains: q, mode: 'insensitive' } }));
    ...
    async search(input) {
        ...
        if (!products.length) {
            ...
            const cameraQuery = input.toLocaleLowerCase().includes('якісна камера') ||
            ...
            const osAndroidQuery = input.toLocaleLowerCase().includes('андроїд') ||
                input.toLocaleLowerCase().includes('андроід') ||
                input.toLocaleLowerCase().includes('android');
            const osIosQuery = input.toLocaleLowerCase().includes('айос') ||
